	...
	Paging.prototype = {
		...
		format: function() {
			...
			if (this.pagecount >8) {
				...
				if (this.current > 6 && this.current <= this.pagecount - 2) {
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><span class="showPage">每页显示</span><select class="ui-select-pagesize"></select><span class="tiao">条</span><span class="jump">跳跃至</span><input type="text" class="ui-paging-count"/><a href="javascript:void(0)">GO</a></li>');
			var sel = $('.ui-select-pagesize', html);
			var str = '';
			for (var i = 0, l = this.settings.pageSizeList.length; i < l; i++) {
				//html+='<li>' +每页显示 + '</li>';
				str += '<option value="' + this.settings.pageSizeList[i] + '">' + this.settings.pageSizeList[i] + '</option>';
			}
			sel.html(str);
